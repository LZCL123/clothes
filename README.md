...
「浮窗衣镜」AI 悬浮虚拟试衣平台后端服务。面向用户提供**拍照/上传照片 → AI 虚拟换装试衣 → 衣服智能推荐**的完整流程,支持微信小程序端。
...
- **用户体系** — 注册登录(短信验证码)、JWT 鉴权、用户信息管理
- **虚拟试衣** — 对接 Leffa 虚拟试衣图像模型,实现换装效果
- **衣服推荐** — 对接推荐模型,结合用户偏好进行个性化推荐
- **文件存储** — MinIO 对象存储 + 阿里云 OSS 双通道,支持图片上传(10MB 单文件 / 100MB 请求)
- **API 文档** — knife4j(OpenAPI 3)自动生成接口文档
- **短信服务** — 阿里云短信验证码(可配置,留空不启用)
...
```
├── clothes-common/    # 通用工具、异常、结果封装
├── clothes-model/     # 实体与 DTO
└── clothes-web/       # Web 入口(控制器、服务、配置)
```
...
- JDK 8+、Maven 3.6+
- MySQL、Redis、MinIO(本地开发可 docker-compose 一键启动)

### 2. 配置环境变量(可选)

所有敏感配置均支持环境变量注入,未配置时使用本地开发默认值:
...
- 数据库脚本与前端源码见姊妹仓库(前端:`clothes-miniapp`)
- 本项目为参赛作品的一部分,AI 图像/推荐模型为团队协作成果,模型源码未包含在本仓库
